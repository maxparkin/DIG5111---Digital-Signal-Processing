...
room_length = 48.81;    % Length in x-direction
room_width  = 98;     % Width in y-direction
room_height = 25.02;     % Height in z-direction
...
absorption_wall   = 0.9;   % Example for left/right and front/back walls
absorption_floor  = 0.3;   % Example for floor
absorption_ceiling= 0.1;   % Example for ceiling
%
...
[audio, audio_fs] = audioread("dry_signal.wav"); % Load audio file.

convolutionSignal = zeros(1, length(audio) + length(h) - 1); % Pre allocate memory to process convolution.

for i = 1:length(audio) % Loop over the length of the array 'audio'.
    for ii = 1:length(h) %Loop over the length of the array 'h' (impulse response).

        % Combine the dry signal with the impulse response.
        convolutionSignal(i + ii - 1) = convolutionSignal(i + ii - 1) + audio(i) .* h(ii);
    end
end

convolutionSignal = 1 * convolutionSignal / max(abs(convolutionSignal)); % Normalise the audio.
audiowrite("convolution_audio.wav", convolutionSignal, audio_fs); % Write audio to a new file.
soundsc(convolutionSignal, audio_fs); % Sound the created convolution signal.
